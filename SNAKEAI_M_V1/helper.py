...
def plot(scores, mean_scores):
    ...
    for i, agent_data in enumerate(all_scores):
        ...
        if i >= len(settings.AGENTS):
            ...
            c = conf["color_head"] # Use head color for the graph line
        ...
        plt.plot(mean_data, linestyle='--', color=color, alpha=0.6, linewidth=1)

        # Text Annotation at the end of the line (Current Score)
        if len(agent_data) > 0:
            last_val = agent_data[-1]
            last_mean = mean_data[-1]
            plt.text(len(agent_data)-1, last_val, str(last_val), color=color, fontsize=9, fontweight='bold')
            plt.text(len(agent_data)-1, last_mean, f"avg:{last_mean:.1f}", color=color, fontsize=8)
