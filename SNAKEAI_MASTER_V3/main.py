...
def train():
    ...
    try:
        while True:
            ...
            for i, snake in enumerate(game.snakes):
                if snake.is_alive:
                    ...
                    if i == 0 and visualizer:
                        visualizer.update(state_old)
            ...
            rewards, dones, current_scores = game.play_step(final_moves, visualizer=visualizer)
        ...
        choice = input(">> Do you want to save this model manually? (y/n): ")
        if choice.lower().strip() == 'y':
            custom_name = input(">> Enter filename (default: model.pth): ").strip()
            if custom_name == "":
                agent.model.save(record=record)
            else:
                agent.model.save(custom_name, record=record)
