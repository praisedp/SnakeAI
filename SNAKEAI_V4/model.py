...
class QTrainer:
    ...
    def train_step(self, state, action, reward, next_state, done, target_model=None):
        ...
        # 2. Get the prediction (Q values) for current state using the main model
        ...
        for idx in range(len(done)):
            Q_new = reward[idx]
            if not done[idx]:
                # Use target network if provided, otherwise fall back to main
                next_pred = target_model(next_state[idx]) if target_model else self.model(next_state[idx])
